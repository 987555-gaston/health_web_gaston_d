...
const MOCK_DOCTORS: Doctor[] = [
  {
    ...
    name: "Dr. Anna Patel",
    specialty: "GP",
    services: ["General Consultation", "Telehealth Appointment"],
  ...
  },
  {
    ...
    services: ["Telehealth Appointment"],
    times: ["Evening"],
    telehealthAvailable: true,
  },
  {
    ...
    name: "Dr. Emma Reed",
    specialty: "GP",
    services: ["General Consultation", "Vaccination", "Telehealth Appointment"],
    times: ["Morning", "Afternoon"],
    telehealthAvailable: true,
  },
];
...
export default function DoctorSearch() {
  ...
  const filtered = useMemo(() => {
    ...
    return MOCK_DOCTORS.filter((d) => {
      ...
      if (specialty && specialty !== "" && d.specialty !== specialty)
        return false;
      if (service && service !== "" && !d.services.includes(service))
        return false;
  ...
  return (
    <div className="container py-8">
      ...
      <p className="mt-2 text-muted-foreground">
        Search by any combination of fields. Fields are optional. Results update
        after you search.
      </p>
        ...
        <CardContent className="p-4 md:p-6">
          ...
          >
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              10% discount applies automatically for 3+ telehealth sessions.
            </p>
          </form>
        </CardContent>
      </Card>
            ...
            <Card key={d.id} className="shadow-sm">
              <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {d.specialty} • {d.services.join(", ")}
                  </p>
                </div>
                <div className="mt-3 sm:mt-0 flex items-center gap-3">
                  <span className="text-sm text-muted-foreground">
                    Available: {d.times.join(", ")}
                  </span>
                  <Button asChild>
                    <a
                      className="h-10 px-4"
                      href={`/confirm?doctor=${encodeURIComponent(d.name)}&specialty=${encodeURIComponent(d.specialty)}&service=${encodeURIComponent(d.services[0])}&date=${encodeURIComponent(date)}&times=${encodeURIComponent(times)}`}
                    >
                      Book
                    </a>
                  </Button>
